...
| :--- | :--- | :--- |
| **Chat Template** | Jinja2 (`chat_template.jinja`) | ChatML (`<|im_start|>role...`) standartında, `tools` şeması enjeksiyonlu ve `add_generation_prompt` destekli şablon. |
| **Harici API (Read)** | Aladhan REST API | Diyanet İşleri vakit hesaplama algoritmasını kullanan canlı REST servis. |
| **Veritabanı (Write/Read)** | SQLite (`islamic_assistant.db`) | Soruları `id`, `topic`, `question`, `user_name`, `created_at` yapısında saklayan yerel DB. |
| **Arayüz (UI)** | Gradio (`app.py`) | Sohbet ekranı, arka plan Trace Log izleyici ve canlı SQLite veritabanı paneli. |
